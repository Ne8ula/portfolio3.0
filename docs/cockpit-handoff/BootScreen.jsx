...
// Lots of machine-like motion: stutter typing, corrupted text resolving,
// fake system errors, scrolling log stream, CRT power cycle, color-shift
// RGB glitch, jittery layout. Timeline is phase-driven.
//
// Phases:
//   init      (0.0s)  — cursor blink on black-ish screen, typed header
//   scroll    (0.4s)  — log lines stream in fast, one every ~45ms
//   stall     (2.6s)  — a line freezes, cursor halts, system hangs
//   crash     (4.7s)  — CRT shutdown (vertical pinch to a single line)
//   dark      (5.4s)  — black screen, faint dot center
//   crton     (6.1s)  — CRT wake-up, horizontal line expands
//   reboot    (6.9s)  — POST check typing, ROM checksum
//   ready     (9.5s)  — final frame with monogram, handoff
//
// External contract: <BootScreen onDone={fn}/> — calls fn() at ~10.5s.
const { useState, useEffect, useRef } = React;

// Typewriter hook — reveals `text` char-by-char at `speed` ms/char.
// Supports a `start` delay and an optional `glitchChars` pool that are
// briefly substituted during reveal, then settle to the real character.
...
function BootScreen({ onDone }) {
  ...
  // Total runtime ≈ 6.0s. Panic phase removed — stall leads straight to crash.
  const T = {
    scroll: 200,
    stall: 1300,
    crash: 2200,
  ...
  // Handoff is now triggered by the user clicking the confirmation button in
  // the ready phase. (Was previously auto-fired at T.done.)
  ...
  // Glitch intensity (0..1). Brief blip in scroll, moderate in stall, heavy in crash.
  ...
  // Jitter offset updates each frame when glitchy
    ...
    >
      ...
      {mainVisible && (
        <div style={{
          ...
          {/* In the 'ready' phase the side/center panels fade out so the
              {A_XIONG} headline + confirm button get the full canvas. */}
          {/* TOP LEFT — header */}
          ...
          {/* CENTER STATE WORD — normally bottom-anchored. In 'ready' phase
              we re-center it so the headline + button sit as one block. */}
          <div style={{
            position: 'absolute',
            left: '50%',
            ...(phase === 'ready'
      ...
      {(phase === 'dark' || phase === 'crton') && (
        <>
          {/* dot in the middle (CRT capacitor bleed) */}
          <div aria-hidden style={{
          ...
          {/* horizontal scan line that expands on crton */}
          {phase === 'crton' && (
            <div aria-hidden style={{
...
function StageList({ phase, now, cursor }) {
  ...
  // Post-reboot override: all but last turn green
      ...
      {stages.map((s, i) => {
        ...
        if (isReboot) {
          ...
          if (!visible) { color = 'var(--ink-faint)'; }
            ...
            color = 'var(--ink)';
            // currently-active = the latest visible one not yet followed by next
            ...
            // After stall time, mark handshake as stalling (yellow/red-ish)
      ...
      <div>
        {lines.map((l, i) => {
          ...
          const style = {
            ...
            filter: corrupt && i > lines.length - 6 ? 'none' : 'none',
          };
  ...
  if (phase === 'stall') {
    return (
      <div>
        <div className="term-display" style={{
          fontSize: 'clamp(64px, 11vw, 150px)',
          color: '#B13A2A', lineHeight: 1,
          animation: 'termJitter 0.12s infinite',
        }}>
          [STALLED{cursor ? '!' : ' '}]
        </div>
...
// ConfirmButton — shown only in the ready phase. User must click to proceed
// to the cockpit. Aesthetic: terminal frame, jade accent, scanning corners,
// a hover-lift, and a small "press ENTER" hint with a blinking cursor.
function ConfirmButton({ onConfirm, cursor }) {
  const [hover, setHover] = React.useState(false);
  const [pressed, setPressed] = React.useState(false);

  // Listen for ENTER / SPACE as a keyboard shortcut.
  React.useEffect(() => {
    const onKey = (e) => {
  ...
  // Corner ticks for a CRT-frame feel
  const Corner = ({ style }) => (
  ...
  return (
    <div style={{
      ...
      animation: 'termFadeIn 0.6s ease-out',
    }}>
      <button
        onClick={click}
      ...
      <div className="term-small" style={{
        color: 'var(--ink-soft)',
        letterSpacing: '.22em',
