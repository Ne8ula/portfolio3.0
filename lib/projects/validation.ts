  ...
  projects.forEach((project, index) => {
    ...
    project.links.forEach((link, linkIndex) => {
      ...
      if (!/^https:\/\/.+/.test(link.href) && !link.href.startsWith('/')) {
        issues.push(
          issue(linkSubject, `href "${link.href}" must be https:// or site-relative`),
        )
      }
    })
    ...
    if (project.cover.kind === 'image') {
      if (!project.cover.src.startsWith(COVER_PREFIX)) {
        issues.push(
          issue(subject, `cover.src must start with "${COVER_PREFIX}", got "${project.cover.src}"`),
        )
      }
