...
| decorations.ts | Hobby props, small/LIT/palette-safe (sax, tablet, gachapon, plant, sitcom note, handheld, shaker). All small + static EXCEPT the shaker — frosted flip-cap bottle (stepped lid, spout, jade accents, half-fill tan shake) at near-crate height (s 2.2) beside the PC, behind the mouse: cockpit click → damped wiggle + liquid slosh (`root.tick`; takes camera+renderer); PC hitbox wins overlapping clicks. `__cockpitDecor.list()`/`.set()`. |
