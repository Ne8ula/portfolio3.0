...
## Dev
`npm run dev` / `build` / `start` / `lint`. Gates: `npm run typecheck:contracts` (strict tsconfig.contracts.json over lib/**+tests), `validate:contracts` (tsx script), `test:unit` (Vitest), `test:e2e` (Playwright Chromium smoke; needs the DEV server — test hooks are compiled out of prod). CI runs all five (.github/workflows/ci.yml). Next.js 16 App Router (Turbopack), React 19, TS with `ignoreBuildErrors`; 3D/HUD files use `@ts-nocheck` (NOT lib/** — strict island). three.js `^0.184` **imperative** (no React Three Fiber — do not convert). `@pmndrs/vanilla` supplies MeshTransmissionMaterial. Runtime deps are intentionally minimal; `postprocessing` + `three-mesh-bvh` remain installed for planned wiring. Tailwind v4 tokens + inline styles with CSS vars. `public/micrographics/`: 70 SVG sticker sheets (few used — see decals.ts).
...
3. **Cockpit** — cursor free-look (±22° yaw, ±15° pitch); camera + objects dead still (only the platter spins); wireframe SVG cursor (cream arrow at rest → jade pointing HAND over anything clickable), no reticle.
...
| lib/projects/catalog.ts (+ project-textures.ts) | CANONICAL `PROJECTS` — 6 REAL projects (Song of Maka, Chu Yu Hong, Tencent Games, NYU Welcome, Shanghai Noir, ProcGen Dungeon), server-safe/serializable (provisional schema + strict `Project` target type + slug/title helpers). `components/cockpit/project-textures.ts` (`client-only`) keeps `makeDiscTexture`. Crate sleeves and deck disc/card read the same catalog; crate depth auto-derives from count. Content edits = canonical-portfolio changes (see Responsive & content system). |
...
| decorations.ts | Hobby props, small/LIT/palette-safe (sax, tablet, plant, handheld, shaker). Static EXCEPT two clickables (`root.tick`; takes camera+renderer; PC hitbox wins): shaker (s 2.2, beside PC) → wiggle + slosh; drawing tablet (flat before the keyboard, yaw = PC): hero-glass shell over an opaque cream liner (REQUIRED — bare transmissive glass shows the void through the see-through desk), charcoal plate PROUD of the rim (transparent decals can't sit under transmissive glass), 5-button strip (leftmost JADE), stylus in gravity pose; click → lifts + scribbles ~1.6s. `__cockpitDecor.list()`/`.set()`. |
...
| cursors.ts / boot-screen / warp-transition / theme-toggle | SVG data-URI cursors (`CURSOR_DEFAULT` cream arrow / `CURSOR_POINTER` jade hand — app-wide via `<main>` in cockpit-app, and on every 3D pick handler + HUD button) · intro phases · toggle. |
...
## Responsive & content system (MANDATORY workflow)
Phase 0 of [docs/hud-responsive-layout-plan.md](docs/hud-responsive-layout-plan.md) is live; [docs/responsive-system.md](docs/responsive-system.md) is the technical contract. Before creating/modifying/porting/reviewing ANY rendered UI, layout, 3D framing, interaction, or canonical portfolio content: read DESIGN.md §10 + docs/responsive-system.md + the relevant `LayoutContract`/`ContentContract` (lib/responsive/, lib/content/); a UI/content task is INCOMPLETE until the five gates pass. Hard rules: canonical facts live ONLY in lib/projects/catalog.ts + lib/portfolio/profile.ts (never JSX/canvas/textures/hover-only); NEVER invent project facts/metrics/outcomes (content authoring = Phase 0A, owner-approved; gaps stay gaps — see docs/content-inventory.md); NEVER create/refresh `content/portfolio-approvals.json` records (owner-only); new randomness takes a seedable named stream arg (§9.6.5); every route needs a co-located layout-contract.ts (validated); `data-hud`/`data-layout-region`/`data-layout-contract`/`data-content-contract` identifiers on new surfaces; deck-overlap bug is Phase 6's — do NOT stopgap. `__COCKPIT_TEST_HOOKS__` (test-hooks.ts) is the dev-only test bridge — additive to `__cockpit*`, never folded in, compiled out of prod.
...
## Next
Phase 0A owner content approval (docs/content-inventory.md) → Phase 0B blocking gates → Phases 1–8 per the plan · VIEW MORE destination (`cockpit-project-view` stub; PROJECTS carry `url`) · Tencent cover art · chatbot behind ScreenDialog · wire `postprocessing` · mobile fallback (separate plan).
